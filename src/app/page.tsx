...
function SyncConfigImport({ onApply }: { onApply: (config: Pick<SyncConfig, "endpoint" | "username" | "password">) => void }) {
  const locale = useContext(LocaleContext);
  ...
  const apply = () => {
    try {
      onApply(parseAgentSyncConfig(value));
      setError("");
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Could not read the SYNC_CONFIG block."); }
  };
  return <details className="sync-import" open><summary>{isChinese ? "粘贴 Agent 返回的 SYNC_CONFIG" : "Paste an agent SYNC_CONFIG result"}</summary><p>{isChinese ? "粘贴完整区块会自动填入服务器地址、用户名和密码。加密口令仍需你自己设置。" : "Paste the complete block to fill the endpoint, username, and password. Set your own encryption passphrase."}</p><textarea value={value} onChange={(event) => setValue(event.target.value)} placeholder="===SYNC_CONFIG_START===\n…\n===SYNC_CONFIG_END===" spellCheck={false} /><button type="button" onClick={apply}>{isChinese ? "读取配置" : "Apply configuration"}</button>{error && <p className="sync-import-error" role="alert">{error}</p>}</details>;
}
...
## Steps
1. Detect cloudflared, existing Cloudflare Tunnels and published hostnames, Docker, Caddy, and Tailscale.
2. If this machine already has a usable Cloudflare Tunnel, use that first. Keep the existing tunnel and configure its ingress to the local WebDAV service. Use its HTTPS hostname as the final URL.
3. If there is no usable Cloudflare Tunnel, prefer Docker Compose. Fall back to native Caddy.
4. Create a Caddy build with github.com/mholt/caddy-webdav and use this Caddyfile.
5. Use Tailscale only when neither a usable Cloudflare Tunnel nor a public domain route is available. Run tailscale serve --bg https / http://localhost:8080 and report the hostname.
6. When Tailscale is selected, tell the user before the final output that every machine using MossChat sync must install Tailscale, sign in to the same tailnet, and be permitted by its ACLs.
7. Generate the bcrypt hash with caddy hash-password, generate a strong password, and verify HTTPS, CORS preflight, PROPFIND, authentication, and a temporary write/delete.
